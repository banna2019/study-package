...
BACKUP_LOG=/data/logs/baklog.logs
...
do
  if [ `ls -A "${LOCAL_BACKUP_DIR}" |wc -l` -eq 0 ];then
     LAST_FILE=${FIRST_BINLOG} ##如果备份目录中没有备份文件则LAST_FILE=FIRST_FILE
  else
     LAST_FILE=`ls -l ${LOCAL_BACKUP_DIR} |tail -n 1 |awk '{print $9}'` ##last_file取序列最大的binlog文件
  fi
  ${BACKUP_BIN} -R --host=${REMOTE_HOST} --user=${REMOTE_USER} --password=${REMOTE_PASS} --raw ${LAST_FILE} -r ${LOCAL_BACKUP_DIR} --stop-never --stop-never-slave-server-id=${SERVER_ID} 
